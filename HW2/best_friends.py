...
def pmi_for_text(unigr_dict, bigr_dict):
    """Retrieveing counts for pmi metric for each bigram"""
    unigram_sum = sum(unigr_dict.values())
    bigram_sum = sum(bigr_dict.values())
    ...
    for bigram in bigr_dict:
        ...
        if unigram_1 >= 10. and unigram_2 >= 10.:   # treshold for disregarding unigrams
            pmi = pmi_metric(unigram_1 / unigram_sum,
                            unigram_2 / unigram_sum,
                            bigram_value / bigram_sum)
            pmi_dict[bigram] = pmi
